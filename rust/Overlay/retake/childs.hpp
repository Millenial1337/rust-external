...
namespace Retake
{
	...
	bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
	{
		...
		parent_window->DrawList->AddRectFilled(ImVec2(ImGui::GetWindowPos().x + 0, ImGui::GetWindowPos().y + 0), ImVec2(ImGui::GetWindowPos().x + size_arg.x + 0, ImGui::GetWindowPos().y + size_arg.y + 0), ImColor(25, 25, 29, 255), 0, 15);
	...
	void RetakeBeginChild(const char* str_id, const ImVec2& size_arg)
	{
		...
		ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(ImGui::GetWindowPos().x + 2, ImGui::GetWindowPos().y + 2),
			ImVec2(ImGui::GetWindowPos().x + size_arg.x - 2, ImGui::GetWindowPos().y + 25), ImColor(30, 30, 35, 255), 0, 15);
